...
public class Sindexing
{
	...
	/**
	 * Insert the rule use info rui in this table in the index x, if there was a rule
	 * use info in the index x then it should be replaced by the merge of rui and the
	 * existed one 
	 * @param x int
	...
	private RuleUseInfoSet insertInIndex (int x,RuleUseInfo rui)
	{
		...
		else
		{
			...
			RuleUseInfo rnew=new RuleUseInfo(r.getSub()
					,r.getPosCount()+rui.getPosCount()
					,r.getNegCount()+rui.getNegCount()
					,r.getFlagNodeSet().union(rui.getFlagNodeSet()));
			ruis.remove(pos);
			ruis.putIn(rnew);
			res.putIn(rnew);
		}
	...
	/**
	 * Get the index of a RuleUseInfo in the table by the id's of its substitutions 
	 * @param x int[]
	 * @return int
	 */
