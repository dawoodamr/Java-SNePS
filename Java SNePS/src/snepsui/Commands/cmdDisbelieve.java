...
public class cmdDisbelieve extends javax.swing.JPanel {
	...
	public cmdDisbelieve(Network network, SNePSInterface frame, JFrame windowFrame) {
		super();
		this.frame = frame;
		this.network = network;
		...
		try {
			...
			if(frame.getsNePSULPanel1().getMenuDrivenCommands().checkConsistency(relationlist, nodelist)) {
				try {
					...
					CaseFrame caseframe = network.getCaseFrame("actObject,action");
